...
static void bringUp(MockStream &s, MatterExtractorHood &fan, uint8_t percent, MatterExtractorHood::FanMode_t mode, MatterExtractorHood::FanModeSequence_t seq) {
  MatterEndPoint::hearthClearDeclarations();
  Hearth.begin(s);
  fan.begin(percent, mode, seq);
  s.expect("AT+MTEP?", "+MTEP:0,1,0x007A\r\nOK\r\n");
  Matter.begin();
}

static void test_begin_declares_and_adopts(void) {
  MockStream s; MatterExtractorHood fan;
  bringUp(s, fan, 0, MatterExtractorHood::FAN_MODE_OFF, MatterExtractorHood::FAN_MODE_SEQ_OFF_HIGH);
  check("declared as extractor_hood", MatterEndPoint::hearthDeclaredTypeAt(0) == 0x007A);
  check("adopted endpoint 1", fan.getEndPointId() == 1);
  check("begin() itself issued no AT traffic", s.scriptDrained());
  check("no unexpected commands", s.unexpected().empty());
  check("initial mode cached", fan.getMode() == MatterExtractorHood::FAN_MODE_OFF);
...
static void test_set_mode_writes_update(void) {
  ...
  check("setMode(HIGH) succeeds", fan.setMode(MatterExtractorHood::FAN_MODE_HIGH));
  check("mode cached", fan.getMode() == MatterExtractorHood::FAN_MODE_HIGH);
  check("no unexpected commands", s.unexpected().empty());
...
static void test_set_mode_rejects_mode_outside_sequence(void) {
  MockStream s; MatterExtractorHood fan;
  bringUp(s, fan, 0, MatterExtractorHood::FAN_MODE_OFF, MatterExtractorHood::FAN_MODE_SEQ_OFF_HIGH);
  check("setMode(LOW) is refused, LOW is not in this sequence", !fan.setMode(MatterExtractorHood::FAN_MODE_LOW));
  check("mode unchanged", fan.getMode() == MatterExtractorHood::FAN_MODE_OFF);
...
static void test_set_on_off_writes_fan_mode_not_a_separate_attribute(void) {
  ...
  check("mode cache shows FAN_MODE_ON, not some other coupled attribute", fan.getMode() == MatterExtractorHood::FAN_MODE_ON);
  check("no unexpected commands", s.unexpected().empty());
  check("script drained", s.scriptDrained());
}
...
static void test_controller_change_mode_fires_callback(void) {
  ...
  MatterExtractorHood::FanMode_t mode = MatterExtractorHood::FAN_MODE_OFF;
  fan.onChangeMode([&](MatterExtractorHood::FanMode_t m) { modeSeen++; mode = m; return true; });
  fan.onChange([&](MatterExtractorHood::FanMode_t, uint8_t) { changeSeen++; return true; });
  s.injectURC("+MTATTR:1,514,0,3");  /* FAN_MODE_HIGH */
  Hearth.poll();
  check("onChangeMode fired", modeSeen == 1 && mode == MatterExtractorHood::FAN_MODE_HIGH);
  check("onChange also fired", changeSeen == 1);
  check("cached mode updated", fan.getMode() == MatterExtractorHood::FAN_MODE_HIGH);
...
static void test_failed_mode_write_returns_false(void) {
  ...
  check("a rejected mode write returns false", !fan.setMode(MatterExtractorHood::FAN_MODE_HIGH));
  check("and does not update the cache", fan.getMode() == MatterExtractorHood::FAN_MODE_OFF);
  check("no unexpected commands", s.unexpected().empty());
}
...
class TypeCheckingFan : public MatterExtractorHood {
public:
  ...
  bool attributeChangeCB(uint16_t endpoint_id, uint32_t cluster_id, uint32_t attribute_id, esp_matter_attr_val_t *val) override {
    ...
    return MatterExtractorHood::attributeChangeCB(endpoint_id, cluster_id, attribute_id, val);
  }
};
...
static void test_rebegin_after_reconcile_does_not_desync_the_cache(void) {
  MockStream s; MatterExtractorHood fan;
  bringUp(s, fan, 40, MatterExtractorHood::FAN_MODE_HIGH, MatterExtractorHood::FAN_MODE_SEQ_OFF_HIGH);

  check("a second begin() after Matter.begin() is refused", !fan.begin(0, MatterExtractorHood::FAN_MODE_OFF, MatterExtractorHood::FAN_MODE_SEQ_OFF_HIGH));
  check("and reports the composition-rejected code", Hearth.lastError() == 10);
  check("the cached mode was not overwritten", fan.getMode() == MatterExtractorHood::FAN_MODE_HIGH);
  ...
  s.expect("AT+MTATTR=1,514,0,0,1", "+MTATTR:1,514,0,0\r\nOK\r\n");
  check("so the next setMode(OFF) really does turn the hood off", fan.setMode(MatterExtractorHood::FAN_MODE_OFF));
  check("and the write happened", s.scriptDrained());
  check("no unexpected commands", s.unexpected().empty());
}

int main(void) {
  printf("\n===== MatterExtractorHood tests =====\n");
