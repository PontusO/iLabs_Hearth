/*
 * FullAPI reference: MatterOnOffPlugin
 *
 ...
 * sketch exercises it. Public surface is identical to MatterOnOffLight's
 * (see that class's header comment); only the device type differs
 * (0x010A, on_off_plugin_unit).
 *
 *   MatterOnOffPlugin()           global object below
...
void setup() {
  ...
  Plugin.begin(false);
  Matter.begin();
  Serial.println("FullAPI MatterOnOffPlugin ready; '?' for menu");
}
