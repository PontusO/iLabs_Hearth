/*
 * FullAPI reference: MatterDishwasher
 *
 ...
 *
 * MatterDishwasher is one of the OperationalState trio
 ...
 * MatterLaundryWasher/MatterLaundryDryer siblings are therefore identical
 * in every particular except the class name and the object's own name;
 * the cross-class proof that two trio members really do share one
 ...
 *
 *   MatterDishwasher()          global object below
 ...
 *   if (!Dishwasher.setOperationalState(MatterDishwasher::kStateRunning)) {
...
void setup() {
  ...
  Dishwasher.begin();  // starts Stopped; declares the endpoint only. The C6's cluster defaults apply until a setter writes state.
  Matter.begin();
  Serial.println("FullAPI MatterDishwasher ready; '?' for menu");
}
...
void loop() {
  ...
  if (Serial.available()) {
    switch (Serial.read()) {
      case '1':
        Serial.println(Dishwasher.setOperationalState(MatterDishwasher::kStateRunning) ? "setOperationalState(Running): OK" : "setOperationalState(Running): failed");
        break;
      case '2':
        Serial.println(Dishwasher.setOperationalState(MatterDishwasher::kStatePaused) ? "setOperationalState(Paused): OK" : "setOperationalState(Paused): failed");
        break;
      case '0':
        Serial.println(Dishwasher.setOperationalState(MatterDishwasher::kStateStopped) ? "setOperationalState(Stopped): OK" : "setOperationalState(Stopped): failed");
        break;
      case 'e':
        Serial.println(Dishwasher.setOperationalState(3) ? "setOperationalState(3): OK (unexpected)" : "setOperationalState(3): failed (expected; Error is reserved)");
        break;
      case 's': Serial.print("operationalState: "); Serial.println(Dishwasher.getOperationalState()); break;
