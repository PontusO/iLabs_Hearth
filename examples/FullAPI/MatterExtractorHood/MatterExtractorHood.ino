/*
 * FullAPI reference: MatterExtractorHood
 *
 ...
 * (0x007A, extractor_hood). On/off has no separate attribute: it is a
 ...
 *   MatterExtractorHood()          global object below
...
void setup() {
  ...
  Serial.print(MatterExtractorHood::MAX_SPEED);
  Serial.print("  MIN_SPEED: ");
  Serial.print(MatterExtractorHood::MIN_SPEED);
  Serial.print("  OFF_SPEED: ");
  Serial.println(MatterExtractorHood::OFF_SPEED);

  Hood.onChangeMode([](MatterExtractorHood::FanMode_t mode) {
    Serial.print("onChangeMode: ");
    Serial.println(MatterExtractorHood::getFanModeString(mode));
    return true;
  });
  Hood.onChangeSpeedPercent([](uint8_t percent) {
    Serial.print("onChangeSpeedPercent: ");
    Serial.println(percent);
    return true;
  });
  Hood.onChange([](MatterExtractorHood::FanMode_t mode, uint8_t percent) {
    Serial.print("onChange: ");
    Serial.print(MatterExtractorHood::getFanModeString(mode));
    Serial.print(" ");
    Serial.println(percent);
    return true;
  });

  Hood.begin(0, MatterExtractorHood::FAN_MODE_OFF, MatterExtractorHood::FAN_MODE_SEQ_OFF_LOW_MED_HIGH_AUTO);
  Matter.begin();
  Serial.println("FullAPI MatterExtractorHood ready; '?' for menu");
}
...
void loop() {
  ...
  if (Serial.available()) {
    switch (Serial.read()) {
      case '1': Serial.println(Hood.setOnOff(true)  ? "on: OK"  : "on: failed");  break;
      ...
      case '+':
        {
          ...
          uint8_t p = (raw > MatterExtractorHood::MAX_SPEED) ? MatterExtractorHood::MAX_SPEED : (uint8_t)raw;
          Serial.print("setSpeedPercent(");
          Serial.print(p);
          Serial.print("): ");
          Serial.println(Hood.setSpeedPercent(p) ? "OK" : "failed");
        }
        break;
      case '-':
        {
          int16_t p = (int16_t)Hood.getSpeedPercent() - 10;
          if (p < MatterExtractorHood::OFF_SPEED) p = MatterExtractorHood::OFF_SPEED;
          Serial.print("setSpeedPercent(");
          Serial.print(p);
          Serial.print("): ");
          Serial.println(Hood.setSpeedPercent((uint8_t)p) ? "OK" : "failed");
        }
        break;
      case 'S':
        {
          ...
          Serial.print(p);
          Serial.print(", silent): ");
          Serial.println(Hood.setSpeedPercent(p, false) ? "OK" : "failed");
        }
        break;
      case 'm':
        {
          uint8_t next = (static_cast<uint8_t>(Hood.getMode()) + 1) % 7;
          MatterExtractorHood::FanMode_t mode = static_cast<MatterExtractorHood::FanMode_t>(next);
          Serial.print("setMode(");
          Serial.print(MatterExtractorHood::getFanModeString(mode));
          Serial.print("): ");
          Serial.println(Hood.setMode(mode) ? "OK" : "failed");
        }
        break;
      case 'g': Serial.print("mode: "); Serial.println(MatterExtractorHood::getFanModeString(Hood.getMode())); break;
      case 'p': Serial.print("speedPercent: "); Serial.println(Hood.getSpeedPercent());                        break;
      case 'u': Hood.updateAccessory(); Serial.println("updateAccessory called");                              break;
