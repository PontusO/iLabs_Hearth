/*
 * MatterExtractorHood.h - actuator clone, C3 of the ten-type swoop.
 *
 * Clones MatterFan.h's public surface onto device type 0x007A,
 * extractor_hood (esp_matter_endpoint.h:132,
 * ESP_MATTER_EXTRACTOR_HOOD_DEVICE_TYPE_ID; the namespace at line 882 wraps
 * cluster::fan_control::config_t, the same FanControl cluster MatterFan
 * already speaks). Cluster 0x0202 is FanControl (FanMode 0x0000, an enum8;
 * PercentSetting 0x0002 and PercentCurrent 0x0003, both uint8), the same
 * IDs MatterFan.cpp reads. There is no connectedhomeip header on a host
 * build to pull the named constants from, so they are given as the plain
 * integers here instead.
 *
 ...
 * device type ID.
 *
 * begin() calls hearthDeclare(this, 0x007A) and caches percent, mode and the
...
class MatterExtractorHood : public MatterEndPoint {
public:
