/*
 * MatterExtractorHood.cpp - implementation. See the header for the design
...
namespace {
/* extractor_hood (esp_matter_endpoint.h:132,
 * ESP_MATTER_EXTRACTOR_HOOD_DEVICE_TYPE_ID), chip::app::Clusters::
 ...
 * constants from. */
const uint32_t kExtractorHoodDeviceType = 0x007A;
...
const char *MatterExtractorHood::fanModeString[7] = { "OFF", "LOW", "MEDIUM", "HIGH", "ON", "AUTO", "SMART" };
const uint8_t MatterExtractorHood::fanModeSequence[6] = {
...
MatterExtractorHood::MatterExtractorHood() {}

MatterExtractorHood::~MatterExtractorHood() {
  end();
}

bool MatterExtractorHood::begin(uint8_t percent, FanMode_t fanMode, FanModeSequence_t fanModeSeq) {
  if (!hearthDeclare(this, kExtractorHoodDeviceType)) {
    return false;
  }
...
void MatterExtractorHood::end() {
  started = false;
}

bool MatterExtractorHood::setMode(FanMode_t newMode, bool performUpdate) {
  if (!started) {
    return false;
  }
 ...
 */
bool MatterExtractorHood::setSpeedPercent(uint8_t newPercent, bool performUpdate) {
  if (!started) {
    return false;
  }
...
bool MatterExtractorHood::setOnOff(bool newState, bool performUpdate) {
  if (!started) {
    return false;
  }
...
bool MatterExtractorHood::getOnOff() {
  return currentFanMode != FAN_MODE_OFF;
}

bool MatterExtractorHood::toggle(bool performUpdate) {
  return setOnOff(!getOnOff(), performUpdate);
}

bool MatterExtractorHood::attributeChangeCB(uint16_t endpoint_id, uint32_t cluster_id, uint32_t attribute_id, esp_matter_attr_val_t *val) {
  bool ret = true;
  if (!started) {
    return false;
  }
...
esp_matter_val_type_t MatterExtractorHood::hearthAttrTypeFor(uint32_t cluster_id, uint32_t attribute_id) const {
