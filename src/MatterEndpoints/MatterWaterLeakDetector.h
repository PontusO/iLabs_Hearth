 ...
 * are a Hearth addition, not present in upstream's MatterWaterLeakDetector.h
 * public section. Every boolean-state sensor in this library (also
 * MatterContactSensor, MatterRainSensor, MatterWaterFreezeDetector) carries
 * the same addition, for the same reason: a sketch needs some way to learn
 * about a controller-driven change, and upstream's own class exposes none.
 * The name is provisional pending a decision on whether to align it with a
 * future upstream addition; see README's "Supported device types" section.
